...
def xpu_device_count():
    return torch.xpu.device_count()


def check_xpu() -> Tuple[bool, str, int, int]:
    # return: is_ddp, device, rank, world_size
    device_count = xpu_device_count()
    if "PMI_SIZE" in os.environ:
        rank = int(os.environ["PMI_RANK"])
        world_size = int(os.environ["PMI_SIZE"])
        assert rank < device_count, f"Not enough devices for local rank {rank}"
        # re-config env
        ...
        return True, f"xpu:{rank}", rank, world_size
    assert device_count >= 1, "Cannot find xpu"
    return False, f"xpu:0", -1, -1
...
def main():
    ...
    use_ddp, device, local_rank, world_size = check_xpu()
    ...
    train_dataloader = DataLoader(
        train_dataset, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size, **extra_args
    )
    ...
    eval_dataloader = DataLoader(
        eval_dataset, collate_fn=default_data_collator, batch_size=args.per_device_eval_batch_size, **eval_extra_args
    )
